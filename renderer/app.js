...
(() => {
  ...
  // Create and unlock AudioContext on EVERY click (Electron/Chrome requirement)
  document.addEventListener('click', () => {
    try {
      if (!_ac) _ac = new (window.AudioContext || window.webkitAudioContext)();
      if (_ac.state !== 'running') _ac.resume().then(() => { _acReady = true; });
      else _acReady = true;
    } catch(e) {}
  });

  // Also try to unlock on mousedown (earlier than click)
  document.addEventListener('mousedown', () => {
    try {
      if (!_ac) _ac = new (window.AudioContext || window.webkitAudioContext)();
      if (_ac.state === 'suspended') _ac.resume().then(() => { _acReady = true; });
    } catch(e) {}
  }, { once: false });

  function playPersonTone(name) {
    if (!_acReady || !_ac) return;          // not unlocked yet
    const now = Date.now();
    if (now - _lastTone < 120) return;      // throttle: max once per 120ms
    _lastTone = now;
    try {
  ...
  function init() {
    ...
    // Fix calendar page — make it normal block (not flex/overflow:hidden)
    const calPage = document.getElementById('page-calendar');
    if (calPage) { calPage.style.display = ''; calPage.style.overflow = ''; calPage.style.height = ''; }
    ...
    for(let d=1;d<=lastD;d++){
      ...
      dsh.forEach(s=>{
        ...
        html+=`<div class="shift-block" style="background:${hexToRgba(pc,.18)};border-left:3px solid ${pc}"
          data-date="${ds}" data-person="${ea(s.person)}"
          onclick="event.stopPropagation();playPersonTone('${ea(s.person)}');window.showShiftCtx(event,'${ds}','${ea(s.person)}')" onmouseover="playPersonTone('${ea(s.person)}')"          draggable="true"          ondragstart="window.onShiftDragStart(event,'${ds}','${ea(s.person)}')"          ondragover="event.preventDefault();event.stopPropagation();this.style.outline='2px solid var(--accent2)'"          ondragleave="this.style.outline=''"          ondrop="this.style.outline='';window.onShiftDropOnBlock(event,'${ds}','${ea(s.person)}')"          title="${eh(s.person)} · ${s.startTime}–${s.endTime}">
  ...
  window.showShiftCtx = function(event, date, person) {
    ...
    document.getElementById('ctx-header').innerHTML =
      `<span style="color:var(--accent)">${eh(person)}</span> &nbsp;·&nbsp; ${date}<br>
  ...
  function openCopyModal(type) {
    ...
    document.getElementById('modal-copy-desc').innerHTML =
      `Kopiraj <strong>${srcShifts.length} smjena</strong> iz <strong>${typeLabel}</strong> u odabrane ciljne mjesece.
       ${person ? `<br><span style="color:var(--accent)">Osoba: ${eh(person)}</span>` : ''}`;
    ...
    // Build month target buttons (12 months from current)
    ...
    for (let i=1; i<=12; i++) {
      ...
      if (m > 11) { m -= 12; y++; }
      const key = `${y}-${pad(m+1)}`;
      const isCurrent = (y===state.calYear && m===state.calMonth);
      const btn = document.createElement('button');
      btn.className = 'copy-month-btn' + (isCurrent?' current-month':'');
      btn.dataset.key = key;
    ...
    // Copy mode toggle
    const modeExact  = document.getElementById('copy-mode-exact');
    const modeChrono = document.getElementById('copy-mode-chrono');
    state._copyMode = 'exact';
    if (modeExact) {
      modeExact.onclick  = () => { state._copyMode='exact';  modeExact.classList.add('active');  modeChrono?.classList.remove('active'); };
      modeChrono.onclick = () => { state._copyMode='chrono'; modeChrono.classList.add('active'); modeExact?.classList.remove('active'); };
      // Reset to exact mode default
      modeExact.classList.add('active'); modeChrono?.classList.remove('active');
    }

    document.getElementById('copy-sel-next').onclick = () => {
      grid.querySelectorAll('.copy-month-btn:not(.current-month)').forEach((b,i)=>{
        if(i===0){b.classList.add('selected');state._copyTargets.add(b.dataset.key);}
      });
    };
    document.getElementById('copy-sel-all').onclick = () => {
      grid.querySelectorAll('.copy-month-btn:not(.current-month)').forEach(b=>{
        b.classList.add('selected'); state._copyTargets.add(b.dataset.key);
      });
    };
  ...
  function confirmCopy() {
    ...
    const chronoMode= document.getElementById('copy-chrono')?.checked;
    const copyMode  = chronoMode ? 'chrono' : 'exact';
    const src       = state._copySrcShifts || [];
    const targets   = [...(state._copyTargets||[])];
    const p         = proj(); if(!p) return;
    ...
    targets.forEach(targetMonthStr => {
      const [ty, tm] = targetMonthStr.split('-').map(Number);

      if (copyMode === 'exact') {
        // Same day number in target month
        src.forEach(s => {
          const srcDay  = new Date(s.date+'T00:00:00').getDate();
          const lastDay = new Date(ty, tm, 0).getDate();
          if (srcDay > lastDay) { skipped++; return; }
          const newDate = `${ty}-${pad(tm)}-${pad(srcDay)}`;
        ...
        // CHRONOLOGICAL: map shifts preserving day-of-week sequence
        // Sort source shifts by date, then map to same weekday slots in target month
        const sorted = [...src].sort((a,b)=>a.date.localeCompare(b.date));
        // Build list of all target month dates by weekday
        const lastDay = new Date(ty, tm, 0).getDate();
        // Map: for each src shift's weekday, find same weekday in target month in order
        const weekdayQueues = {}; // 0-6 -> [dates in target month]
        for (let d=1; d<=lastDay; d++) {
          const ds  = `${ty}-${pad(tm)}-${pad(d)}`;
          const dow = new Date(ds+'T00:00:00').getDay();
          if (!weekdayQueues[dow]) weekdayQueues[dow] = [];
          weekdayQueues[dow].push(ds);
        }
        const weekdayUsed = {}; // track which index we've used for each weekday

        sorted.forEach(s => {
          const dow = new Date(s.date+'T00:00:00').getDay();
          const queue = weekdayQueues[dow] || [];
          const usedIdx = weekdayUsed[dow] || 0;
          if (usedIdx >= queue.length) { skipped++; return; }
          const newDate = queue[usedIdx];
          weekdayUsed[dow] = usedIdx + 1;
          const exists = p.shifts.find(x=>x.date===newDate && x.person===toPerson);
          if (exists && !overwrite) { skipped++; return; }
          if (exists) p.shifts = p.shifts.filter(x=>!(x.date===newDate&&x.person===toPerson));
          p.shifts.push({ ...s, date:newDate, person:toPerson, _manual:true });
    ...
    showToast(`📋 Kopirano ${added} smjena u ${targets.length} mj.${skipped?` (${skipped} preskočeno)`:''}!`);
  }
  ...
  function setupExportPage() {
    ...
    document.getElementById('btn-export-pdf').onclick = async ()=>{
      if(!selPerson()){alert('Odaberi osobu!');return;}
      const html=Exporter.buildPrintHTML(shifts(),selPerson(),state.calYear,state.calMonth,getAvatar(selPerson()));
  ...
  function applySettingsUI(){
    ...
    // notif24/notif1 default true unless explicitly set to false
    if (n24) n24.checked = (s.notif24 !== false);
    if (n1)  n1.checked  = (s.notif1  !== false);
    // Sync settings state with DOM values (in case data loaded without UI)

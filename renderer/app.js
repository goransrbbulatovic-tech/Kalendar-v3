...
(() => {
  ...
  function confirmCopy() {
    const persons   = [...(state._copySelectedPersons||[])];
    const overwrite = document.getElementById('copy-overwrite')?.checked ?? false;
    const src       = state._copySrcShifts || [];
    const targets   = [...(state._copyTargets||[])];
    const p         = proj(); if(!p) return;
    const chrono    = (state._copyMode === 'chrono');
    ...
    persons.forEach(person => {
      const personSrc = src.filter(s => s.person === person);
      if (!personSrc.length) return;
      ...
      targets.forEach(targetMonthStr => {
        ...
        if (chrono) {
          // ── HRONOLOŠKI: prati dan-u-sedmici ─────────────────────
          // Sort source shifts by date
          const sorted = [...personSrc].sort((a,b)=>a.date.localeCompare(b.date));
          
          // Get the source month start day-of-week
          const [sy, sm] = sorted[0].date.slice(0,7).split('-').map(Number);
          const srcMonthFirstDow = new Date(sy, sm-1, 1).getDay(); // 0=Sun
          const tgtMonthFirstDow = new Date(ty, tm-1, 1).getDay();
          
          sorted.forEach(s => {
            const srcDay  = new Date(s.date+'T00:00:00');
            const srcDow  = srcDay.getDay();    // day of week of this shift
            const srcDayN = srcDay.getDate();   // day number in source month
            
            // Find which weekday occurrence this is in the source month
            // e.g. 2nd Monday of May
            const weekOfMonth = Math.floor((srcDayN - 1) / 7); // 0,1,2,3,4
            
            // In target month: find the same weekday occurrence
            // First occurrence of same weekday in target month
            let firstSameDow = 1 + ((srcDow - tgtMonthFirstDow + 7) % 7);
            let targetDay    = firstSameDow + weekOfMonth * 7;
            
            // If that day overflows the month, go back one week
            if (targetDay > lastDayOfTarget) targetDay -= 7;
            if (targetDay < 1) { skipped++; return; }
            
            const newDate = `${ty}-${pad(tm)}-${pad(targetDay)}`;
            
            // Skip if another person already has this date+person combo
            ...
            p.shifts.push({...s, date:newDate, person, _manual:true});
            added++;
          });

        } else {
          // ── TAČNA KOPIJA: isti dan broja ─────────────────────────
          personSrc.forEach(s => {
            const srcDay = new Date(s.date+'T00:00:00').getDate();
            if (srcDay > lastDayOfTarget) { skipped++; return; }
      ...
      // Add person to people list if new
    ...
    showToast(`📋 Kopirano ${added} smjena u ${targets.length} mj. za ${persons.length} osoba${skipped?` (${skipped} preskočeno)`:''}`);
  }

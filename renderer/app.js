...
(() => {
  ...
  function renderCalendar() {
    ...
    for(let d=1;d<=lastD;d++){
      ...
      dsh.forEach(s=>{
        ...
        html+=`<div class="shift-block" style="background:${hexToRgba(pc,.18)};border-left:3px solid ${pc}"
          ...
          onclick="event.stopPropagation();window.showShiftCtx(event,'${ds}','${ea(s.person)}')"          draggable="true"          ondragstart="window.onShiftDragStart(event,'${ds}','${ea(s.person)}')"          ondragover="event.preventDefault();event.stopPropagation();this.style.outline='2px solid var(--accent2)'"          ondragleave="this.style.outline=''"          ondrop="this.style.outline='';window.onShiftDropOnBlock(event,'${ds}','${ea(s.person)}')"          title="${eh(s.person)} · ${s.startTime}–${s.endTime}">
    ...
    // Attach hover sound to all shift blocks AFTER render (JS, not inline HTML)
